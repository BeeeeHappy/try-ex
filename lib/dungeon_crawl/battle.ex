defmodule DungeonCrawl.Battle do
  ...
  defp attack(%{hit_points: 0}, character_b), do: character_b
  ...
  defp attack_message(character = %{name: "You"}, damage),
    do: "You attack with #{character.attack_description} " <> "and deal #{damage} damage."

  defp attack_message(character, damage),
    do:
      "#{character.name} attacks with " <>
        "#{character.attack_description} and " <> "deals #{damage} damage."

  defp receive_message(character = %{name: "You"}, damage),
    do: "You receive #{damage}. Current HP: #{character.hit_points}."

  defp receive_message(character, damage),
    do: "#{character.name} receives #{damage}. " <> "Current HP: #{character.hit_points}."
end
